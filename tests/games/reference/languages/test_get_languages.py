...
@pytest.mark.smoke
class TestLanguages:
    @allure.title(f'{Cases.GAMES["TG94"]["id"]} Test languages list')
    ...
    @allure.title(f'{Cases.GAMES["TG98"]["id"]} Test languages read')
    @allure.description('Проверка успешного ответа [200] при запросе языка по ID.')
    @allure.testcase(name=Cases.GAMES["TG98"]["name"], url=Cases.GAMES["TG98"]["link"])
...
class TestLanguagesRegression:
    @allure.title(f'{Cases.GAMES["TG97"]["id"]} Test read a language with a non-existent ID')
    ...
    @allure.title(f'{Cases.GAMES["TG96"]["id"]} Test read a language with a deleted ID')
