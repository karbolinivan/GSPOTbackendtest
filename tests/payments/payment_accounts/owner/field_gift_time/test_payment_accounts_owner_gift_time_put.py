...
class TestPaymentPutOwner:
    ...
    @allure.title('Test payment accounts owner update valid value param gift_time')
    @allure.description('Проверка успешного ответа [200] при изменении gift_time владельца валидными значениями')
    @pytest.mark.parametrize("value", ["0", "11:25:45", "1", "12 11:05:33.666666"])
    def test_payment_accounts_owner_positive_input_valid_value_param_gift_time_put(self, value):
        """Изменение параметра  gift_time владельца валидными значениями"""
        print("Запрос PUT")
        payload = {
            "commission": "08",
...
class TestPaymentPutOwnerRegression:
    ...
    def test_payment_accounts_owner_positive_input_invalid_value_param_gift_time_put(self, value):
        ...
        expected = ExpectedJSON.key_value(key="gift_time",
                                          value=ExpectedJSON.PAYMENT_ACCOUNTS_OWNER_INVALID_VALUE_GIFT_TIME.value)
