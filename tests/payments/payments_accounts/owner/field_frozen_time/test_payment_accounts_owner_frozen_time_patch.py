...
class TestPaymentPatchOwner:
    ...
    @allure.title('Test payment accounts owner partial update valid value param frozen_time')
    @allure.description('Проверка успешного ответа [200] при частичном обновлении frozen_time валидными значениями')
    @pytest.mark.parametrize("value", ["0", "11:25:45", "1", "12 11:05:33.666666"])
    def test_payment_accounts_owner_positive_input_valid_value_param_frozen_time_patch(self, value):
        """Частичное обновление параметра  frozen_time владельца валидными значениями"""
        print("Запрос PATCH")
        payload = {
            "commission": "08",
            "frozen_time": f'{value}',
